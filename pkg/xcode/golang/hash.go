...
func computeHash(contents []byte, hash []byte) []byte {
	...
	//compute new hash
	hash = hashutil.ComputeCryptoHash(newSlice)
...
func hashFilesInDir(rootDir string, dir string, hash []byte, tw *tar.Writer) ([]byte, error) {
	...
	for _, fi := range fis {
		...
		if tw != nil {
			...
			if err = WriteStreamToPackage(is, fqp, filepath.Join("src", name), tw); err != nil {
...
func generateHashcode(spec *pb.XCodeSpec, tw *tar.Writer) (string, error) {
	if spec == nil {
		return "", fmt.Errorf("Cannot generate hashcode from nil spec")
	}

	xcodeID := spec.XcodeID
	if xcodeID == nil || xcodeID.Path == "" {
	...
	hash := hashutil.GenerateHashFromSignature(actualcodepath, ctorbytes)
